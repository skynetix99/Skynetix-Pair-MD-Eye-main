...
> © POWERED BY SKYNETIX MD TEAM`;
    ...
    try {
        ...
        await sock.sendMessage(chatId, { text: repoText }, { quoted: msg });
        console.log('[repo] Message sent successfully to:', chatId);
    } catch (error) {
        console.error('[repo] Failed to send message:', error.message);
        
        // Fallback: Send without quoting
        try {
            await sock.sendMessage(chatId, { text: repoText });
            console.log('[repo] Message sent without quote successfully');
        } catch (fallbackError) {
            console.error('[repo] All send attempts failed:', fallbackError.message);
            throw fallbackError;
        }
    }
};    };
