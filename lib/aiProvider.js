...
function nonEmptyAnswer(value, providerName) {
    ...
    if (Array.isArray(candidate)) {
        candidate = candidate.map((item) => item?.text || item?.content || item).join(' ');
    }
    if (candidate && typeof candidate === 'object') {
        candidate = candidate.text || candidate.content || candidate.value || candidate.output || candidate.result;
    }
    const answer = typeof candidate === 'string' ? candidate.trim() : String(candidate || '').trim();
    if (!answer || /^no response$/i.test(answer)) throw new Error(`${providerName}: empty response`);
    return answer;
}

function extractAnswer(data, providerName) {
    const candidates = [
    ...
    for (const candidate of candidates) {
        try { return nonEmptyAnswer(candidate, providerName); } catch {}
    }
...
async function askOpenAI(userJid, userMessage) {
    ...
    return extractAnswer(data, 'OpenAI');
}
...
async function askGateway(userJid, userMessage) {
    ...
    return extractAnswer(data, 'AI Gateway');
}
