...
async function getAIResponse(userJid, userMessage) {
    ...
    const errors = [];
    const providers = [
        ['OpenAI', () => askOpenAI(userJid, question)],
        ['AI Gateway', () => askGateway(userJid, question)],
        ['Pollinations', () => askPollinations(question)]
    ];
    ...
    throw new Error(`No AI provider is available. Configure a valid OPENAI_API_KEY or AI_GATEWAY_URL/AI_GATEWAY_KEY. Details: ${errors.join(' | ')}`);
}
